        ...
        <div className="space-y-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button onClick={fetchProducts} className="text-zinc-600 hover:text-ruby transition-colors">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button
                        ...
                        className="bg-ruby text-black px-8 py-3 rounded-sm text-[10px] uppercase tracking-[0.2em] font-bold hover:bg-white transition-all shadow-lg shadow-ruby/10 flex items-center gap-2"
                    >
            ...
            {isLoading ? (
                <div className="py-20 flex flex-col items-center justify-center space-y-4">
                    <Loader2 size={40} className="text-ruby animate-spin" />
                    <p className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold">Synchronizing Manifest...</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {filteredProducts.map((product) => (
                        <div key={product.id} className="bg-zinc-900/30 border border-zinc-800/50 rounded-sm overflow-hidden group hover:border-ruby/30 transition-all duration-500 flex flex-col">
                            <div className="relative aspect-[4/5] bg-zinc-900 overflow-hidden">
                                ...
                                <div className="absolute top-4 left-4 flex gap-2">
                                    <span className={`px-3 py-1 rounded-full text-[8px] uppercase tracking-widest font-bold backdrop-blur-md border ${product.is_featured ? 'bg-ruby/10 text-ruby border-ruby/20' : 'bg-black/40 text-zinc-400 border-white/5'
                                        }`}>
                                ...
                                <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                                    <span className="text-[8px] uppercase tracking-[0.2em] text-ruby font-bold">{product.category}</span>
                                </div>
                            </div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <p className="text-[9px] text-ruby/60 font-mono mt-1">Value: {product.numeric_price}</p>
                                        </div>
                                    </div>
